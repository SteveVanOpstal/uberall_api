//
// AUTO-GENERATED FILE, DO NOT MODIFY!
//
// @dart=2.18

// ignore_for_file: unused_element, unused_import
// ignore_for_file: always_put_required_named_parameters_first
// ignore_for_file: constant_identifier_names
// ignore_for_file: lines_longer_than_80_chars

import 'package:openapi/api.dart';
import 'package:test/test.dart';

// tests for AcceptedExtraFieldsResponseObject
void main() {
  // final instance = AcceptedExtraFieldsResponseObject();

  group('test AcceptedExtraFieldsResponseObject', () {
    // List of accepted allergens
    // List<String> allergens (default value: const [])
    test('to test the property `allergens`', () async {
      // TODO
    });

    // List of accepted dietary restrictions
    // List<String> dietaryRestrictions (default value: const [])
    test('to test the property `dietaryRestrictions`', () async {
      // TODO
    });
  });
}
